...
$con=new ConnectDb();
$sql4 = "select *  from annal where id=".$id;
...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="#6699CC" style="border-collapse:collapse">
    <tr>
        ...
        <td width="94" bgcolor='#EBE2FE'>厨师</td>
        <td width="94" bgcolor='#EBE2FE'>配送员</td>
        <td width="204" bgcolor='#EBE2FE'>订单数量</td>
        <td width="86" bgcolor='#EBE2FE'>所负金额</td>
        <td width="86" bgcolor='#EBE2FE'>留言</td>
    </tr>
    <?php
    $sql="select *  from annal where 1=1 and asch like '$asch'";
    ...
    if(@$page!=""){
        $page_size=6;
        $sql1="select count(*) as total from annal where 1=1 and asch like '$asch'";
    ...
    <tr>
        ...
        <td> <?php echo $res2['achus'];?></td>
        <td> <?php echo $res2['apenc'];?></td>
...
<p align="center"> <?php echo "<a href='annal_list.php?page=1'>首页</a>&nbsp";
    if($page!=1){
        $go=$page-1;
        echo "<a  href='annal_list.php?page=$go'>上一页</a>&nbsp";
    }
    for($i=1;$i<=$page_count;$i++){
        echo "<a id='$i' href='annal_list.php?page=$i' value='$i'>$i</a>&nbsp";
    }
    $focus=@$_GET['page'];
    if($page<$page_count){
        $back=$page+1;
        echo "<a href='annal_list.php?page=$back'>下一页</a>&nbsp";
    }
    echo "<a href='annal_list.php?page=$page_count'>尾页</a>&nbsp";?></p>
