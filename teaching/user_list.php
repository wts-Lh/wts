...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="#6699CC" style="border-collapse:collapse">
    ...
    <?php
    $sql="select * from user where zt='入职' order by id desc";
    //查询并显示分页
    $con=new ConnectDb();
    @$page=$_GET['page'];
    if(@$page==""){
...
<p align="center"> <?php echo "<a href='user_list.php?page=1'>首页</a>&nbsp";
    if($page!=1){
        $go=$page-1;
        echo "<a  href='user_list.php?page=$go'>上一页</a>&nbsp";
    }
    for($i=1;$i<=$page_count;$i++){
        echo "<a id='$i' href='user_list.php?page=$i' value='$i'>$i</a>&nbsp";
    }
    $focus=@$_GET['page'];
    if($page<$page_count){
        $back=$page+1;
        echo "<a href='user_list.php?page=$back'>下一页</a>&nbsp";
    }
    echo "<a href='user_list.php?page=$page_count'>尾页</a>&nbsp";?></p>
