...
<table width="100%" border="1" align="center" cellpadding="3" cellspacing="1" bordercolor="#6699CC" style="border-collapse:collapse">
    ...
    <?php
    $sql="select * from annal where azt='未完成' ";
    if (@$_POST["bh"]!="")
    {
        $nreqbh=@$_POST["bh"];
...
<p align="center"> <?php echo "<a href='annal_updt.php?page=1'>首页</a>&nbsp";
    if($page!=1){
        $go=$page-1;
        echo "<a  href='annal_updt.php?page=$go'>上一页</a>&nbsp";
    }
    for($i=1;$i<=$page_count;$i++){
        echo "<a id='$i' href='annal_updt.php?page=$i' value='$i'>$i</a>&nbsp";
    }
    $focus=@$_GET['page'];
    if($page<$page_count){
        $back=$page+1;
        echo "<a href='annal_updt.php?page=$back'>下一页</a>&nbsp";
    }
    echo "<a href='annal_updt.php?page=$page_count'>尾页</a>&nbsp";?></p>
